...
def fit_function(
	...
	prior_residual = np.array([np.log(params["w0"]) - np.log(params["w0_prior"]), np.log(params["w0"]) - 2*np.log(params["w1"])]) / params["tau_prior"]
...
def simulate_matrix(params, pake_data, intrinsic_lineshape, t, r):
	...
	# start = int((1 + n) * pake_r.shape[0] // 2) - 1
	# end = int((-1 + n) * pake_r.shape[0] // 2)

	start_idx = int((1 + n) * N_field // 2) - 1
	end_idx = int((-1 + n) * N_field // 2)

	# Handle negative end_idx for slicing (standard python slicing handles -x from end)
	# If end_idx is exactly 0, slice [start:]? No, end_idx is approx -1024.
...
def do_fitting(
	...
	params = lmfit.create_params(
		...
		alpha_plus_beta=dict(value=0.1, vary=True, min=0.05, max=1),  # noqa: C408
		...
		w0=dict(  # noqa: C408
			...
			vary=True,
			min=0.1,
			max=2,  # N414Q works reasonably well with max at 1 but max is not good
		),
		w0_prior=dict(value=1 / 2.355, vary=False),
		tau_prior=dict(value=2, vary=False),
		...
		r0=dict(  # noqa: C408
			expr="r1 + delta if r1 + delta <= 7 else 7",
			# value=5.0,
			# vary=True,
			# # value=4.5,
			vary=False,
...
class FittingWorker(QtCore.QThread):
	...
	def callback_relay(self, params, iter, resid, *args, **kwargs):
		...
		val = float(np.log10(np.linalg.norm(resid)))
