...
def main(filename):
    ...
    if True:
        ...
        for ind, col in enumerate(cols):
            ...
            try:
                ...
                dat[:, ind] = coldat[
                    center - int((h - l) / 2) : center + int((h - l) / 2) + 1
                ]
    ...
    ratioE = [
        E[i] / E[i + 1] if i < len(E) - 1 else 0 for i, _ in enumerate(E)
    ]
    ratioE = ratioE[:-2]
    ...
    ah.legend(markerfirst=False, handlelength=1)
    ...
    ax.legend(markerfirst=False, handlelength=1)
