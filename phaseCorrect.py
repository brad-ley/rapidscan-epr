...
def phase(_, alldata, ref_file):
    ...
    def loop(col):
        try:
            ...
            phi = np.angle(
                np.dot(np.conjugate(sig)[0:stop], numpyref[0:stop]))
            sig *= np.exp(1j * phi)
...
def process(set_progress, _, filepath, ref_file, skiprows):
    try:
        files = [
            ...
            if "_".join(P(filepath).stem.split("_")[:-1]) in str(ii)
...
app.layout = html.Div(
    [
        ...
        html.Div([
            dcc.Graph(id='phased', figure=fig, style={'height': graph_height})
        ],
                 style={
                     'width': '98%',
                     'align': 'center'
