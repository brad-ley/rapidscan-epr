...
def triple_gaussian(x, x0, w0, x1, w1, x2, w2, a, b):
    return (1 - b) * double_gaussian(x, x0, w0, x1, w1, a) + b * gaussian(
        x, x2, w2
    )


def gaussian(x, x0, w0):
    return (1 / (w0 * np.sqrt(2 * np.pi))) * np.exp(
        -1 / 2 * ((x - x0) / w0) ** 2
    )
...
def ret(pars, simulations, single, r):
    ...
    o = np.convolve(
        single,
        lin_combo(
            simulations, triple_gaussian(r, r0, w0, r1, w1, r2, w2, a, 0)
        ),
        mode="same",
    )
...
def fit_fun(x, simulations, to_fit, r, single, fitidx):
    ...
    return ((to_fit - ret(x, simulations, single, r)) ** 2)[
        fitidx[0] : fitidx[1]
    ] * windows.blackman(len(to_fit))
...
def proc(
    ...
    obj = lmfit.Minimizer(
        fit_fun, params, fcn_args=(dists, spectrum, r, single, fitidx)
    )
...
def main(filename, ri, rf, numplots=-1):
    ...
    _mat = feather.read_feather(
        "/Users/Brad/Library/CloudStorage/GoogleDrive-bdprice@ucsb.edu/My Drive/Research/Data/2024/3/21/283.1 K/105mA_23.6kHz_pre30s_on30s_off240s_10000avgs_filtered_batchDecon.feather"
    )  # single-label dataset to get the non-dipolar broadened data
        ...
        for _ind, _col in enumerate(_cols):
            ...
            try:
                _adjusted_spectra[_ind, :] = _coldat[
                    _center - int((_h - _l) / 2) : _center + int((_h - _l) / 2)
                ]
            except ValueError:
                _adjusted_spectra[_ind, :] = _coldat[
    ...
    t = np.linspace(
        0, 2 * adjusted_spectra.shape[0], adjusted_spectra.shape[0]
    )
    ...
    params = lmfit.create_params(
        r0=dict(value=2.8, vary=True, min=2.1, max=5),
        w0=dict(value=0.5, vary=False, min=0.25, max=1.5),
        r1=dict(value=5, vary=True, min=2.1, max=7),
        ...
        b=dict(value=0.0, vary=False, min=0, max=1),
        amp=dict(
            value=np.max(adjusted_spectra[-1, :]) / np.max(interp_dists),
            vary=True,
        ),
    ...
    inds = range(
        0,
        len(adjusted_spectra_zeros[:, 0]),
        ceil(len(adjusted_spectra_zeros[:, 0]) / num),
    )
    ...
    with mp.Pool(processes=mp.cpu_count()) as pool:
        p = pool.map(
            partial(
                proc, dists=interp_dists, r=r, params=params, single=single
            ),
            tqdm(
                adjusted_spectra_zeros[
                    :: ceil(len(adjusted_spectra_zeros[:, 0]) / num), :
                ]
            ),
        )
    ...
    for ind in range(0, len(p), ceil(len(p) / numplots)):
        ...
        out = ret(res.params, interp_dists, single, r)  # type: ignore
        ax.plot(
            B,
            out / np.max(spectrum) + c,
            label=f"Fit",
            c=line.get_color(),
            ls="--",
        )
    ...
    dt.to_csv(
        P(filename).parent.joinpath(
            P(filename).stem + "_pake-gaussian-fits_single.txt"
        ),
        index=False,
    )
...
def plot(filename, numplots=-1):
    ...
    singlestr = (
        dres.loc[dres["name"] == "single"]["value"]
        .values[0]
        .lstrip("[array([")
        .rstrip("])")
        .split(",")
    ...
    for ind in range(0, len(expts), ceil(len(expts) / numplots)):
        ...
        ax.plot(
            r,
            np.max(times) / numplots * (np.max(times) * g / np.sum(g) + c),
        )
    ...
    fraca = np.array(
        [
            float(
                dres.loc[(dres["time_pt"] == exp)]
                .loc[dres.loc[(dres["time_pt"] == exp)]["name"] == "a"][
                    "value"
                ]
                .values[0]
            )
    ...
    fracb = np.array(
        [
            float(
                dres.loc[(dres["time_pt"] == exp)]
                .loc[dres.loc[(dres["time_pt"] == exp)]["name"] == "b"][
                    "value"
                ]
                .values[0]
            )
    ...
    pre = float(
        "".join(
            [
                ii
                for ii in "".join(
                    [i for i in P(filename).stem.split("_") if "pre" in i]
                )
                if ii.isdigit()
            ]
        )
    ...
    on = float(
        "".join(
            [
                ii
                for ii in "".join(
                    [i for i in P(filename).stem.split("_") if "on" in i]
                )
                if ii.isdigit()
            ]
        )
    ...
    ag.axvspan(
        pre, pre + on, facecolor="#00A7CA", alpha=0.25, label="Laser on"
    )
    ag.scatter(
        np.linspace(np.min(times), np.max(times), len(fraca)),
        fraca,
        c="red",
        label="a frac",
    )
    ag.scatter(
        np.linspace(np.min(times), np.max(times), len(fracb)),
        fracb,
        c="green",
        label="b frac",
    )
    ag.scatter(
        np.linspace(np.min(times), np.max(times), len(fracb)),
        1 - (1 - np.array(fraca)) * (1 - np.array(fracb)),
        c="k",
        label="Unfolded %",
    )
    ...
    fig.savefig(
        P(filename).parent.joinpath(
            P(filename).stem + "_distVtime_single.png"
        ),
        dpi=600,
    )
    fg.savefig(
        P(filename).parent.joinpath(
            P(filename).stem + "_fracVtime_single.png"
        ),
        dpi=600,
    )
...
if __name__ == "__main__":
    ...
    main(filename, ri=1.2, rf=8, numplots=10)
    plot(filename, numplots=10)
    plt.show()
