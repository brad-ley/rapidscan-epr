...
def gaussian(x, x0, w):
    return (1 / (w * np.sqrt(2 * np.pi))) * np.exp(
        -1 / 2 * ((x - x0) / w) ** 2
    )
...
def fit_fun(x, simulations, to_fit, r):
    ...
    return ((to_fit - ret(x, simulations, r)) ** 2) * windows.general_hamming(
        len(to_fit), 0.75
    )
...
def main(filename):
    ...
    for ind, col in enumerate(cols):
        peak = (
            np.argmax(
                spectra[col][
                    len(spectra[col]) // 10 : 9 * len(spectra[col]) // 10
                ]
            )
            + len(spectra[col]) // 10
        )
        ...
        if peak > B_center:
            adjusted_spectra[ind, :] = spectra[col][
                peak - B_edge : peak + B_edge
            ]
        elif peak <= B_center:
            adjusted_spectra[ind, :] = spectra[col][
                peak - B_edge : peak + B_edge
        ...
        adjusted_spectra[ind, :] -= (
            np.mean(adjusted_spectra[ind, :32])
            + np.mean(adjusted_spectra[ind, :32])
        ) / 2
    ...
    _data = np.loadtxt(
        P(
            "/Users/Brad/Library/CloudStorage/GoogleDrive-bdprice@ucsb.edu/My Drive/Research/Code/dipolar averaging/results64_room_T_0.75_-7.72.txt"
        ),
        delimiter=",",
    )
    ...
    params = lmfit.create_params(
        ...
        amp=dict(
            value=np.max(adjusted_spectra[-1, :])
            / np.max(interp_dists[len(_data[1:, 0]) // 2]),
            min=0,
        ),
    ...
    c = 0
    for ind in tqdm(range(0, len(adjusted_spectra[:, 0]), 1)):
        spectrum = adjusted_spectra[ind, :]
        obj = lmfit.Minimizer(
            fit_fun, params, fcn_args=(interp_dists, spectrum, r)
        )
        ...
        a.plot(
            r,
            gaussian(r, res.params["r0"].value, res.params["w0"].value)
            + 10 * c,
            label="ampgo",
        )
