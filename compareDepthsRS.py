...
class Fitfile:
    ...
    def tau(self):
        return float(self.instr[1].split(", ")[2]), float(
            self.instr[4].split(", ")[2]
        )
...
def main(folder):
    subfolders = sorted(
        [
            ...
            if ii.is_dir() and checkfloat(ii.name)
        ]
    )

    outstr = f"Temp (K), Depth (G), Efficiency (%), Time (s), Time error (s)\n"
    for sf in subfolders:
        fitfile = [ii for ii in sf.iterdir() if ii.name == "LWfit-values.txt"][
            0
        ]
        v = Fitfile()
        v.load(fitfile)
