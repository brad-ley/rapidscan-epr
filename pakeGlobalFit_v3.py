...
def fit_function(
	...
	window = np.repeat(
		windows.tukey(resid.shape[0], 0.1)[:, np.newaxis],
		resid.shape[1],
		axis=1,
	)
	# window = np.repeat(
	...
	prior_residual = np.array([np.log(params["w0"]) - np.log(params["w0_prior"]), np.log(2*params["w0"]) - np.log(params["w1"])]) / params["tau_prior"]
...
def do_fitting(
	...
	params = lmfit.create_params(
		...
		w0_prior=dict(value=0.5 / 2.355, vary=False),
