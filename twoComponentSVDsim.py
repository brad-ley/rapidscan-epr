...
def main():
    ...
    t2_light = 50
    pct_change = 2
    g1 = (99 * np.ones(len(t)) - pct_change * np.heaviside(t - t1_light, 0.5) * np.exp(-(t - t1_light) / 100))
    g2 = (1 * np.ones(len(t)) + pct_change * np.heaviside(t - t2_light, 0.5) * np.exp(-(t - t2_light) / 100))
    ...
    for i in range(k):
        a.plot(B, V[i, :] / V[i, np.argmax(np.abs(V[i, :]))] + i * 0.1, label=f'$C_{{{i+1}}}$')
    p = V[0, :] + V[1, :]
    p /= p[np.argmax(np.abs(p))]
    # a.plot(B, p, label='add first 2', ls=':')
    a.plot(B, (l1-l2) / (l1-l2)[np.argmax(np.abs(l1-l2))], ls='--', label='Diff')
    n = 2
    ...
    h.plot([1 if i >= len(E) - 1 else E[i]/E[i+1] for i, _ in enumerate(E)], label='ratio')
    h.set_yscale('log')
    h.legend()
