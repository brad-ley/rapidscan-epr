...
def makePlot(targ):
    ...
    for i, f in enumerate(files):
        ...
        bh = -20
        dnoise = np.std(
            dat['abs'][np.logical_and(dat['B'] > bl, dat['B'] < bh)])
        dpk = np.max(dat['abs'])
        dSNR = dpk / dnoise
        """ DECON NOISE """
        plotfield = 12
        ...
        label = f.stem.replace('oneFileDecon', '').replace('holder','').replace('_',' ')
        ...
        ax.text(1.25e-5 * 1e6, np.mean(rawdat[:3] + 0.01),
                f'({int(SF(SNR,3))})', horizontalalignment='right')
